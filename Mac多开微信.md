...
> **部署目标**：3 个微信（1 主 + 2 分身）。本机最新验收状态见 `ROADMAP.md`。
> **当前创建入口**：`bash 升级分身.command --create-only`；拒绝覆盖已有分身。新版桌面启动器启动时也会创建缺失分身，并自动同步较新的主微信版本；运行中的分身延后更新。自动同步与失败恢复的完整行为见 `README.md`。
> **4.1.15（270100）兼容修正**：须保留主程序沙盒权限并映射分身应用身份／应用组。先重签内嵌组件，再只给最外层主程序应用权限，不把这份权限与 `--deep` 混用。下文手工 Phase 6 的旧签名步骤不适用于该版本，请使用已更新脚本；签名通过不代表启动通过。
...
所以多开需要克隆主微信、改 Bundle ID 和显示名、配置更新策略，并按当前版本正确处理签名与沙盒权限。
